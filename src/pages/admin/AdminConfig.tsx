...
import { Save, ArrowLeft, Image as ImageIcon, Eye, Upload, Trash2, Loader2 } from 'lucide-react';
...
export const AdminConfig = () => {
    ...
    // Estados para archivos locales seleccionados
    ...
    // Función clave para limpiar: resetea estado local y visual
    const handleClearSlide = (slideNum: number) => {
        if (slideNum === 1) {
            setFile1(null);
            setPreview1("");
            setConfig(prev => ({ ...prev, heroImagenUrl: "", heroTitulo: "", heroSubtitulo: "" }));
        } else {
            setFile2(null);
            setPreview2("");
            setConfig(prev => ({ ...prev, heroImagenUrl2: "", heroTitulo2: "", heroSubtitulo2: "" }));
        }
        toast.success(`Slide ${slideNum} limpiado. Guardá para aplicar cambios.`);
    };
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 1. Si hay archivos nuevos en PC, los subimos a Cloudinary primero
            if (file1) {
            ...
            // 2. Preparamos el objeto EXACTO para enviar (con "" si se borró algo)
            const dataToSave = {
                Id: 1,
            ...
            setConfig(prev => ({ ...prev, heroImagenUrl: finalUrl1, heroImagenUrl2: finalUrl2 }));
            setFile1(null);
            setFile2(null);
    ...
    const labelClass = "block text-[10px] font-bold text-brand-muted uppercase mb-1 tracking-widest text-left";
    const uploadZoneClass = "border-2 border-dashed border-brand-light/40 rounded-xl p-4 text-center bg-gray-50 hover:bg-brand-light/10 transition cursor-pointer relative group h-24 flex items-center justify-center";

    return (
        <div className="min-h-screen bg-gray-50 p-8 pb-32 font-body">
            <SEO title="Configuración de Diseño" description="Configuración del Hero y portada" />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 text-left">
                    ...
                    <form onSubmit={handleSave} className="bg-white p-8 rounded-2xl shadow-sm border border-brand-light/20 space-y-8">
                        ...
                        <div className="bg-gray-50/50 p-6 rounded-xl border border-brand-light/20 space-y-6">
                            <div className="flex justify-between items-center">
                                <h3 className="font-display text-xl text-brand-dark flex items-center gap-2">
                                    <span className="bg-brand-primary text-white text-[10px] px-2 py-1 rounded-md font-bold">01</span>
                                    Portada Principal
                                </h3>
                                <button type="button" onClick={() => handleClearSlide(1)} className="text-red-500 hover:bg-red-50 p-2 rounded-lg transition-colors">
                                    <Trash2 size={18} />
                                </button>
                            </div>
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className={labelClass}>Imagen de Fondo (Desde PC)</label>
                                    <div className={uploadZoneClass}>
                                        ...
                                        <div className="flex flex-col items-center">
                                            <Upload className="w-6 h-6 text-brand-muted group-hover:text-brand-primary mb-1 transition-colors" />
                                            <span className="text-[10px] text-brand-muted font-bold uppercase tracking-tighter">Seleccionar archivo</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="bg-gray-50/50 p-6 rounded-xl border border-brand-light/20 space-y-6">
                            <div className="flex justify-between items-center">
                                <h3 className="font-display text-xl text-brand-dark flex items-center gap-2">
                                    <span className="bg-brand-primary text-white text-[10px] px-2 py-1 rounded-md font-bold">02</span>
                                    Segunda Portada
                                </h3>
                                <button type="button" onClick={() => handleClearSlide(2)} className="text-red-500 hover:bg-red-50 p-2 rounded-lg transition-colors">
                                    <Trash2 size={18} />
                                </button>
                            </div>
                            <div className="space-y-4">
                    ...
                    <div className="space-y-6">
                        <h3 className="font-display text-2xl text-brand-dark flex items-center gap-2 px-2 uppercase tracking-tighter">
                        ...
                        <div className="bg-white p-5 rounded-2xl shadow-sm border border-brand-light/20">
                            <p className={labelClass}>Slide 01</p>
                            <div className="h-64 w-full rounded-xl overflow-hidden relative bg-brand-dark shadow-inner">
                                <img 
                                    src={preview1 || config.heroImagenUrl || "https://placehold.co/1280x720/1a1a1a/ffffff?text=Sin+Imagen"} 
                                    className="w-full h-full object-cover opacity-60" 
                                    alt="Preview 1" 
                                />
                                <div className="absolute inset-0 flex flex-col items-center justify-center text-white p-4 text-center">
                                    <span className="font-display text-3xl uppercase tracking-tighter">{config.heroTitulo || "TÍTULO VACÍO"}</span>
                                    <span className="text-[9px] tracking-[0.3em] font-bold mt-4 bg-brand-light text-brand-dark px-4 py-2 rounded-sm uppercase">
                                        {config.heroSubtitulo || "Subtítulo vacío"}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Preview 2 */}
                        <div className="bg-white p-5 rounded-2xl shadow-sm border border-brand-light/20">
                            <p className={labelClass}>Slide 02</p>
                            <div className="h-64 w-full rounded-xl overflow-hidden relative bg-brand-dark shadow-inner">
                                <img 
                                    src={preview2 || config.heroImagenUrl2 || "https://placehold.co/1280x720/1a1a1a/ffffff?text=Sin+Imagen"} 
                                    className="w-full h-full object-cover opacity-60" 
                                    alt="Preview 2" 
                                />
                                <div className="absolute inset-0 flex flex-col items-center justify-center text-white p-4 text-center">
                                    <span className="font-display text-3xl uppercase tracking-tighter">{config.heroTitulo2 || "TÍTULO VACÍO"}</span>
                                    <span className="text-[9px] tracking-[0.3em] font-bold mt-4 bg-brand-light text-brand-dark px-4 py-2 rounded-sm uppercase">
                                        {config.heroSubtitulo2 || "Subtítulo vacío"}
                                    </span>
                                </div>
                            </div>
                        </div>
