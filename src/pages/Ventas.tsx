...
export const VentasPage = () => {
  ...
  useEffect(() => {
    ...
    getActivas().then(data => {
        const ventas = data.filter((p: any) => p.estadoOperacion === "Venta" || !p.estadoOperacion);
            ...
            <div className="flex flex-col lg:flex-row gap-12">
                ...
                <main className="lg:w-3/4">
                    <div className="mb-10 border-b border-brand-light/20 pb-6">
                        ...
                        <p className="text-brand-muted text-sm font-body italic">Descubrí {filtradas.length} propiedades seleccionadas para vos.</p>
                    </div>
                        ...
                        {filtradas.map(prop => (
                            <Link to={`/propiedad/${prop.id}`} key={prop.id} className="bg-white rounded-2xl shadow-sm hover:shadow-[0_20px_50px_rgba(0,0,0,0.1)] transition-all duration-500 border border-brand-light/10 overflow-hidden group flex flex-col">
                                <div className="h-64 relative overflow-hidden shrink-0">
                                    ...
                                    <span className="absolute top-4 right-4 px-3 py-1 rounded text-[10px] font-bold uppercase z-10 text-white shadow-sm bg-brand-primary tracking-widest">
                                        VENTA
                                    </span>
                                </div>
                    ...
                    {filtradas.length === 0 && (
                        <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-brand-light/40">
                            <p className="text-brand-muted font-display text-xl italic">No encontramos propiedades que coincidan con tu búsqueda.</p>
                        </div>
                    )}
                </main>
            </div>
